...
function t:__newindex(k, v)
    ...
    if k > math.floor(Screen.height / 25) then table.remove(self, 1) end
end
...
function LuaLoaderLog.ClearLog() LuaLoaderLog.__log = {} end
...
local function runcommand()
    ...
    LuaLoaderLog.__lasttexts[#LuaLoaderLog.__lasttexts + 1] =
        LuaLoaderLog.__text
    LuaLoaderLog.__text = ''
end
...
hook.Add('OnGUI', 'LuaLoaderLog', function()
    ...
    GUI.Box(Rect(x - 10, y - 20, Screen.width / 2, Screen.height / 1.5),
            'Lua Console')
    ...
    LuaLoaderLog.__text = GUI.TextField(Rect(x - 5, y, w - 10, h),
                                        LuaLoaderLog.__text)
...
hook.Add('OnUpdate', 'LuaLoaderLog', function()
    ...
    if LuaLoaderLog.__gui then
        ...
        if e and r then runcommand() end
...
hook.Add('OnLuaError', 'LuaLoaderLog', function(err) LuaLoaderLog.Error(err) end)

command.Add('clear', function() LuaLoaderLog.ClearLog() end)
