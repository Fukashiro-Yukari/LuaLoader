...
function string.split(str, delimiter)
    ...
    if delimiter == '' then
        ...
        for substr in string.gmatch(str .. delimiter, '(.-)' ..
                                        (((tmp > 96 and tmp < 123) or
                                            (tmp > 64 and tmp < 91) or
                                            (tmp > 47 and tmp < 58)) and
                                            delimiter or '%' .. delimiter)) do
...
function string.tovalue(str) return string.fromHex(str:gsub('%x', '0%1')) end
...
function string.Explode(separator, str, withpattern)
    ...
    for i = 1, string_len(str) do
        local start_pos, end_pos = string_find(str, separator, current_pos,
                                               not withpattern)
...
function string.Split(str, delimiter) return string.Explode(delimiter, str) end
...
-----------------------------------------------------------]]
function string.Implode(seperator, Table) return table.concat(Table, seperator) end
...
-----------------------------------------------------------]]
function string.GetExtensionFromFilename(path) return path:match("%.([^%.]+)$") end
...
function meta:__add(vel) return self .. vel end
