...
function table.getmax(a)
    ...
    for k, v in pairs(a) do if isnumber(v) and v > max then max = v end end
...
function table.getmin(a)
    ...
    for k, v in pairs(a) do if isnumber(v) and v < min then min = v end end
...
--[[---------------------------------------------------------
	...
	Desc: Empty a table
-----------------------------------------------------------]]
function table.Empty(tab) for k, v in pairs(tab) do tab[k] = nil end end
...
function table.Add(dest, source)
    ...
    for k, v in pairs(source) do table.insert(dest, v) end
...
-----------------------------------------------------------]]
function table.SortDesc(t) return table.sort(t, function(a, b) return a > b end) end
...
function table.LowerKeyNames(Table)
    ...
    for k, v in pairs(Table) do
        ...
        -- Recurse
        if (istable(v)) then v = table.LowerKeyNames(v) end
...
function table.CollapseKeyValue(Table)
    ...
    for k, v in pairs(Table) do
        ...
        if (istable(Val)) then Val = table.CollapseKeyValue(Val) end
...
function table.ClearKeys(Table, bSaveKey)
    ...
    for k, v in pairs(Table) do
        if (bSaveKey) then v.__key = k end
...
local function toKeyValues(tbl)
    ...
    for k, v in pairs(tbl) do table.insert(result, {key = k, val = v}) end
...
function SortedPairsByMemberValue(pTable, pValueName, Desc)
    ...
    for k, v in pairs(sortedTbl) do v.member = v.val[pValueName] end
...
function RandomPairs(pTable, Desc)
    ...
    for k, v in pairs(sortedTbl) do v.rand = math.random(1, 1000000) end
...
function table.ForEach(tab, funcname)
    for k, v in pairs(tab) do funcname(k, v) end
end
