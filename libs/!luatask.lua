...
function sys.wait(ms)
    ...
    -- 调用core的rtos定时器
    if 1 ~= _G['__luataskthis']:StartTimer(timerid, ms) then print('__luataskthis.StartTimer error') return end
    -- 挂起调用的任务线程
    local message = {coroutine.yield()}
    if #message ~= 0 then
        _G['__luataskthis']:StopTimer(timerid)
...
function sys.timerStop(val, ...)
    ...
    if type(val) == 'number' then
        ...
        _G['__luataskthis']:StopTimer(val)
    else
...
function sys.timerStart(fnc, ms, ...)
    ...
    --调用底层接口启动定时器
    if _G['__luataskthis']:StartTimer(msgId, ms) ~= 1 then print('__luataskthis.StartTimer error') return end
...
function sys.tigger(param)
    if param < TASK_TIMER_ID_MAX then
        ...
        --如果是循环定时器，继续启动此定时器
        if loop[param] then _G['__luataskthis']:StartTimer(param, loop[param]) end
    end
end
...
function sys.async(ass,method,data,cb)
    ...
    if type(data) == 'table' then
        id = _G['__luataskthis']:AsyncRun(ass,method,table.unpack(data))
    elseif data == nil then
        id = _G['__luataskthis']:AsyncRun(ass,method)
    else
        id = _G['__luataskthis']:AsyncRun(ass,method,data)
    end
